file_path = 'C:\\Users\\CSILLER\\code\\aoc\\2k23_pyipynb\\input\\day05.txt'
...
for seeds in [[[x, x] for x in sds], [[sds[e], sds[e] + sds[e + 1] - 1] for e in range(0, len(sds), 2)]]:
    for seed_min, seed_max in seeds:
        ...
        for block in data:
            arrangements = [[int(x) for x in y.split()]
                            for y in block.splitlines()[1:]]
            next_ranges = []
                ...
                for dest, source, rng in arrangements:
                    if source <= start and source + rng > start:
                        if end - start < rng:
                            ...
                            next_ranges.append(
                                (dest + (start - source), dest + (start + rng - 1 - source)))
